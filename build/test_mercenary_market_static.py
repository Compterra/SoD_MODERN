...
def test_supply_demand_and_report_helpers_exist() -> None:
    ...
    role_fit = read("src/scripts/ZY_helper_scripts/sod_merc_market_calculate_guild_role_fit.py")
    contract_roster = read("src/scripts/ZY_helper_scripts/sod_merc_guild_get_contract_roster.py")
    ...
    assert_contains(guardrails, "script_sod_slavers_apply_market_delta")
    ...
    assert_contains(report_summary, "@Serving {s63}: renewal in {reg26} day(s); unpaid wages {reg27} denars.")
    assert_contains(report_summary, "weekly fee {reg25}, debt {reg24}, relation {reg23}")
    ...
    assert_contains(bid, "script_sod_merc_market_calculate_guild_role_fit")
    assert_contains(bid, "sod_merc_contract_role_supply_column")
    assert_contains(bid, "role_fit_score")
    assert_contains(role_fit, "slot_faction_black_army_contract_heat")
    assert_contains(role_fit, "slot_faction_conquistador_supply_stock")
    assert_contains(role_fit, "slot_faction_elephant_guard_slaver_alarm")
    assert_contains(role_fit, "slot_faction_serpent_intelligence")
    assert_contains(bid, "slot_faction_serpent_safe_passage")
    assert_contains(bid, "slot_faction_slaver_market_heat")
    assert_contains(role_fit, "slot_faction_boar_frontier_pressure")
    ...
    assert_contains(guild_weight, "script_sod_merc_market_calculate_guild_role_fit")
    assert_contains(guild_weight, ":demand_type")
    assert_contains(preferred_guild, ":demand_type")
    assert_contains(contract_roster, "script_sod_merc_guild_get_roster")
    assert_contains(contract_roster, "black_army_tier_1_unit_3")
    ...
    assert_contains(spawn, "script_sod_merc_guild_get_contract_roster")
    assert_contains(spawn, "script_sod_merc_market_calculate_guild_supply")
    assert_contains(spawn, ":specialist_units")
    ...
    assert_contains(return_contract, "ai_bhvr_hold")
    assert_not_contains(return_contract, '(eq, ":assigned", 0)')
    patrol_write = return_contract.index('(party_set_ai_behavior, ":cur_party", ai_bhvr_patrol_party)')
    fallback_branch = return_contract.index("(else_try)", patrol_write)
    hold_write = return_contract.index('(party_set_ai_behavior, ":cur_party", ai_bhvr_hold)')
    assert patrol_write < fallback_branch < hold_write
