...
def test_classic_employer_rotation_uses_market_scoring() -> None:
    legacy = read("src/scripts/ZY_helper_scripts/cf_merc_guild_give_new_employer.py")

    assert_contains(legacy, "script_cf_sod_merc_guild_uses_classic_employer_rotation")
    assert_contains(legacy, "script_sod_merc_market_calculate_kingdom_demand")
...
def test_supply_demand_and_report_helpers_exist() -> None:
    ...
    player_company_count = read("src/scripts/ZY_helper_scripts/sod_count_player_merc_companies.py")
    player_company_predicate = read("src/scripts/ZY_helper_scripts/cf_sod_player_has_active_merc_company_from_guild.py")
    guild_progression = read("src/scripts/ZY_helper_scripts/merc_describe_guild_progression.py")
    report_summary = read("src/scripts/ZY_helper_scripts/merc_describe_report_summary.py")
    ...
    assert_contains(demand, "script_sod_get_center_population_capacity_profile")
    assert_contains(demand, "slot_faction_sod_merc_population_shortage")
    assert_contains(demand, "slot_faction_sod_merc_lord_wealth_score")
    assert_contains(demand, "slot_faction_sod_merc_gold_manpower_pressure")
    assert_contains(demand, ":gold_manpower_pressure")
    assert_contains(demand, "slot_troop_wealth")
    ...
    assert_contains(budget, ":average_lord_wealth")
    assert_contains(budget, ":wealth_surplus")
    assert_contains(budget, ":ruler_surplus")
    ...
    assert_contains(demand_report, "Population shortage")
    assert_contains(demand_report, "lord-gold strength")
    assert_contains(demand_report, "gold-for-manpower pressure")
    ...
    assert_contains(player_quote, "slot_party_sod_merc_contract_quality")
    assert_contains(player_quote, "slot_party_sod_merc_contract_replenishment_level")
    assert_contains(player_quote, "$g_sod_merc_player_hire_blocked")
    assert_contains(player_quote, "$g_sod_merc_player_hire_market_limit")
    ...
    assert_contains(player_quote, '(gt, ":mercs", 0)')
    assert_contains(player_quote, 'troop_add_gold, "trp_player", "$merc_cost"')
    assert_contains(player_quote, 'slot_troop_merc_bought, 0')
    assert player_quote.index('(gt, ":mercs", 0)') < player_quote.index('slot_faction_sod_merc_treasury')
    assert_contains(player_company_count, "try_for_parties")
    assert_contains(player_company_count, "slot_party_type, spt_player_mercenaries")
    assert_contains(player_company_count, "slot_party_boss, \"trp_player\"")
    assert_contains(player_company_count, "slot_party_sod_merc_contract_guild")
    assert_contains(player_company_count, "slot_party_orginal_faction")
    assert_contains(player_company_count, "slot_party_sod_merc_contract_term_end")
    assert_contains(player_company_count, "(assign, reg0, \":company_count\")")
    assert_contains(player_company_predicate, "slot_party_type, spt_player_mercenaries")
    assert_contains(player_company_predicate, "slot_party_sod_merc_contract_guild")
    assert_contains(player_company_predicate, "slot_party_orginal_faction")
    assert_not_contains(guild_progression, "troop_get_slot, \":player_company\", \"trp_player\", slot_troop_mercenaries")
    assert_contains(guild_progression, "script_cf_sod_player_has_active_merc_company_from_guild")
    assert_contains(report_summary, "script_sod_count_player_merc_companies")
    assert_contains(report_summary, "Field companies")
    assert_contains(report_summary, "Kingdom service")
    assert_contains(report_summary, "Guild pact")
    assert_contains(contract_board, "script_sod_count_player_merc_companies")
    assert_contains(contract_board, "Kingdom service")
    assert_contains(contract_board, "Guild pact")
    assert_contains(contract_board, "Field companies")
    ...
    assert_contains(hire_dialog, "$g_sod_merc_player_hire_market_limit")
    assert_contains(hire_accept, "$g_sod_merc_player_hire_blocked")
    assert_contains(hire_accept, "$g_sod_merc_player_hire_market_limit")
    ...
    assert_contains(accept, ":collectable_wealth")
    assert_contains(accept, ":remaining_payment")
    assert_contains(accept, "slot_troop_wealth")
    assert_contains(accept, "troop_set_slot")
    assert_contains(accept, "eq, \":remaining_payment\", 0")
    ...
    assert_contains(accept, "slot_party_sod_merc_contract_quality")
    assert_contains(accept, "slot_party_sod_merc_contract_replenishment_level")
    ...
    assert_contains(merc_lord_outcome, "slot_faction_sod_merc_price_pressure")
    assert_contains(merc_lord_outcome, "slot_party_sod_merc_contract_employer")
    assert_contains(merc_lord_outcome, "slot_party_sod_merc_contract_loss_score")
    assert_contains(merc_lord_outcome, "store_relation")
    assert_contains(merc_lord_outcome, "set_relation")
    assert_contains(merc_lord_outcome, ":relation_loss")
    assert_contains(merc_lord_outcome, ":relation_gain")
    ...
    assert_contains(status_report, "Mercenary Service Board")
    assert_contains(status_report, "guild market ledger")
    assert_contains(status_report, "mnu_mercenary_market_report")
    assert_contains(gm_market_option, "How does the guild market look")
