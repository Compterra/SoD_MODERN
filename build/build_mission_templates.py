# -*- coding: utf-8 -*-
"""
Build compile/module_mission_templates.py from src/mission_templates fragments (vanilla-compatible).

Policy:
- Strict manifest ordering via src/mission_templates/_order_mission_templates.txt.
- One mission template per fragment file, exporting MISSION_TEMPLATES = [ (...), ].
"""
...
SRC = ROOT / "src" / "mission_templates"
OUT = ROOT / "compile" / "module_mission_templates.py"

# Incremental build cache (v42): skip regeneration if inputs unchanged
CACHE_SCHEMA_VERSION = 1
CACHE_DIR = ROOT / ".buildcache"
CACHE_FILE = CACHE_DIR / "mission_templates_manifest.json"
...
ORDER_FILE = SRC / "_order_mission_templates.txt"
...
def _syntax_check_fragment(fp: Path, raw: str) -> None:
    try:
        ...
        raise SystemExit(f"[build_mission_templates] FAIL: {rel}:{lineno}:{offset} {msg}\n" + "\n".join(snippet))
...
def load_preamble_lines() -> List[str]:
    """Load preamble lines from src/mission_templates/_preamble/*.py, if any."""
...
def read_order() -> List[Path]:
    if not ORDER_FILE.exists():
        raise SystemExit(f"Missing mission templates order file: {ORDER_FILE}")
    files: List[Path] = []
    for ln in ORDER_FILE.read_text(encoding="utf-8", errors="replace").splitlines():
        ...
        p = SRC / ln.replace("\\", "/")
        if not p.exists():
            raise SystemExit(f"Mission template fragment listed but missing: {p}")
        files.append(p)
    return files
...
def build(use_cache: bool = True, emit_source_map: bool = True) -> None:
    ...
    if use_cache and OUT.exists():
        ...
        if prev == cache_payload:
            print('[build_mission_templates] Up-to-date; skipped (cache)')
            return
    ...
    for fp in files:
        ...
        if "MISSION_TEMPLATES" not in raw:
            continue
        inner, start_ln, end_ln = extract_list_block(raw, "MISSION_TEMPLATES")
        mid = extract_first_id(inner)
        if mid:
            if mid in seen:
                raise SystemExit(f"Duplicate mission template '{mid}':\n  {seen[mid]}\n  {fp}")
            seen[mid] = fp
        block = inner.rstrip()
        if block and not block.rstrip().endswith(","):
            block += ","
        rel = fp.relative_to(SRC).as_posix()
        label = f"# [ src/mission_templates/{rel}:L{start_ln}-L{end_ln} ]"
        if mid:
            label += f" {mid}"
        if emit_source_map:
    ...
    print(f"[build_mission_templates] Wrote {OUT}")
