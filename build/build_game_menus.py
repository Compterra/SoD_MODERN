# -*- coding: utf-8 -*-
"""
Build compile/module_game_menus.py from src/menus fragments (vanilla-compatible).

Policy:
- Strict manifest ordering via src/menus/_order_game_menus.txt.
- One menu per fragment file, exporting MENUS = [ (...), ].
"""
...
CACHE_FILE = CACHE_DIR / "menus_manifest.json"
...
ORDER_FILE = SRC / "_order_game_menus.txt"
...
def _syntax_check_fragment(fp: Path, raw: str) -> None:
    ...
    try:
        ...
        raise SystemExit(f"[build_game_menus] FAIL: {rel}:{lineno}:{offset} {msg}\n{sn}")
...
def read_order() -> List[Path]:
    if not ORDER_FILE.exists():
        raise SystemExit(f"Missing game menus order file: {ORDER_FILE}")
    files: List[Path] = []
    seen: set[str] = set()
    for ln in ORDER_FILE.read_text(encoding="utf-8", errors="replace").splitlines():
        ...
        rel = ln.replace("\\", "/")
        if rel in seen:
            raise SystemExit(f"Duplicate entry in game menus order file: {rel}")
        seen.add(rel)
        p = SRC / rel
        if not p.exists():
            raise SystemExit(f"Game menu fragment listed but missing: {p}")
        files.append(p)
    ...
    if missing:
        ...
        raise SystemExit(
            "Game menu manifest is incomplete. Unlisted fragment(s):\n"
            f"  {preview}{suffix}"
        )
    return files
...
def build(use_cache: bool = True, emit_source_map: bool = True) -> None:
    ...
    for fp in files:
        ...
        label = f"# [ src/menus/{rel}:L{start_ln}-L{end_ln} ]"
        if mid:
            label += f" {mid}"
        if emit_source_map:
