...
# [ src/presentations/0020_sod_fief_management/sod_fief_management.py:L1-L1375 ] sod_fief_management
...
#############################################################
		...
		(eq, "$pres_sod_fief_info", 1),
			  ...
			  (try_for_range, ":improvement_no", 1, ":count"),
				...
				(try_begin),
				  ...
				  (str_store_string, s22, "@{s68}"),
				(else_try),
				  (str_store_string_reg, s97, s22),
				  (str_store_string, s22, "@{s97}, {s68}"),
				(try_end),
			  (try_end),
			  ...
			  (try_begin),
				...
				(assign, reg6, 1),
				(str_store_string_reg, s97, s24),
				(str_store_string, s24, "@{s97}^{s4}"),
			  (try_end),
		...
		(eq, "$pres_sod_fief_info", 1),		
			  ...
			  (else_try),
			    ...
			    (str_store_string_reg, s68, s0),
			    (str_store_string, s40, "@Upgrade available from {s68}."),
			  (else_try),
			    (eq, ":validation_code", 3),
			    (call_script, "script_get_building_display_name", ":validation_blocking_slot"),
			    (str_store_string_reg, s68, s0),
			    (str_store_string, s40, "@Already built: {s68}."),
			  (else_try),
			    (eq, ":validation_code", 4),
			    (call_script, "script_get_building_display_name", ":validation_blocking_slot"),
			    (str_store_string_reg, s68, s0),
			    (str_store_string, s40, "@Missing prerequisite: {s68}."),
			  (else_try),
			    (eq, ":validation_code", 5),
			    (call_script, "script_get_building_display_name", ":validation_blocking_slot"),
			    (str_store_string_reg, s68, s0),
			    (str_store_string, s40, "@Conflicts with existing improvement: {s68}."),
			  (else_try),
			  ...
			  (try_begin),
				...
				(assign, reg3, 1),
				(str_store_string, s68, "@you reckon"),
			  (else_try),
				(assign, reg3, 0),
				(call_script, "script_store_troop_name", s33, ":max_skill_owner"),
				(str_store_string, s68, "@{s33} reckons"),
			  (try_end),
			  (try_begin),
				(gt, reg12, 0),
				(str_store_string, s69, "@, for an estimated {reg12} week project"),
			  (else_try),
				(str_clear, s69),
			  (try_end),
		
		(create_text_overlay, reg1, "@{s39}^^As the party member with the highest engineer skill ({reg2}), {s68} that developing the {s34} will require {reg10} labor. This center can currently provide {reg11} labor per week{s69}. Cost: {reg5} denars after the local construction cost factor ({reg13}%). You currently have {reg7} denars.^^{s40}", tf_center_justify|tf_scrollable),
        (position_set_x, pos1, 50),
        (position_set_y, pos1, 100),
        (overlay_set_position, reg1, pos1),
		(position_set_x, pos1, 630),
...
("ponavosa_commander_duel", prsntf_read_only, 0, [
  (ti_on_presentation_load,
   [
     ...
     (str_store_troop_name, s68, ":ally_troop"),
     (str_store_troop_name, s69, ":enemy_troop"),
     ...
     (create_text_overlay, "$g_ponavosa_duel_ally_name", s68, 0),
     (position_set_x, pos1, 70),
     ...
     (create_text_overlay, "$g_ponavosa_duel_enemy_name", s69, tf_right_align),
  ...
  (ti_on_presentation_run,
   [
     ...
     (else_try),
       ...
       (str_store_string, s68, "@Layer {reg2} | {reg4}%"),
       (str_store_string, s69, "@Layer {reg3} | {reg5}%"),
       (overlay_set_text, "$g_ponavosa_duel_ally_layer", s68),
       (overlay_set_text, "$g_ponavosa_duel_enemy_layer", s69),
       ...
       (str_store_string, s70, "@Blessing pressure: {reg6} / {reg7}"),
       (overlay_set_text, "$g_ponavosa_duel_blessing", s70),
       ...
       (try_begin),
         (gt, ":dist", 2500),
         (str_store_string, s71, "@Duel time: {reg1}s - the ring is breaking!"),
         (overlay_set_color, "$g_ponavosa_duel_status", 0xFF4500),
       (else_try),
         (gt, ":dist", 2000),
         (str_store_string, s71, "@Duel time: {reg1}s - close the distance."),
         (overlay_set_color, "$g_ponavosa_duel_status", 0xFFD27A),
       (else_try),
         (str_store_string, s71, "@Duel time: {reg1}s - the armies hold their breath."),
         (overlay_set_color, "$g_ponavosa_duel_status", 0xFFFFFF),
       (try_end),
       (overlay_set_text, "$g_ponavosa_duel_status", s71),
     (else_try),
       (presentation_set_duration, 0),
     (try_end),
   ]),
