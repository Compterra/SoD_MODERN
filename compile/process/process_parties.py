...
# KUBA - parties can now be exported from .txt file (check module_parties.py for details)
tag_uses = load_tag_uses(export_dir)
save_tag_uses(export_dir, tag_uses)
if copy_parties_from_txt_file == 0:
	save_python_header(parties)
	save_parties(parties)
	print("Exporting parties")
else:
	try:
		open(export_dir + "parties.txt","w").write(open("parties.txt","r").read())
		print("Parties copied from txt file.")
	except:
		print("Parties.txt not found. Set copy_parties_from_txt_file to 0 in module_parties.py")
