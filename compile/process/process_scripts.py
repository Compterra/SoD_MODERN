...
import module_scripts as module_scripts

scripts = get_canonical_scripts(getattr(module_scripts, "scripts", getattr(module_scripts, "SCRIPTS", [])))
...
def save_scripts(variable_list,variable_uses,scripts,tag_uses,quick_strings):
  ...
  for i_script in range(len(scripts)):
    ...
    if len(func) > 1 and callable(func[1]):
      file.write("%s -1\n"%(convert_to_identifier(func[0])))
      save_statement_block(file,convert_to_identifier(func[0]), 0,[], variable_list,variable_uses,tag_uses,quick_strings)
      file.write("\n")
      continue
