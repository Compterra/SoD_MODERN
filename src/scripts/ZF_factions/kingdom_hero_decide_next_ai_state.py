...
#Recruiting troops ################################
 ...
 #raid villages  #################################		
        (try_begin),  #tr49
          ...
          (assign, ":sod_marshal_planning", 35),
          (assign, ":sod_marshal_aggression", 45),
          (try_begin),
            (is_between, ":faction_no", kingdoms_begin, kingdoms_end),
            (faction_get_slot, ":sod_campaign_posture", ":faction_no", slot_faction_sod_campaign_posture),
            (faction_get_slot, ":sod_marshal_planning", ":faction_no", slot_faction_sod_marshal_planning_score),
            (faction_get_slot, ":sod_marshal_aggression", ":faction_no", slot_faction_sod_marshal_aggression_score),
          (try_end),
          ...
          (try_for_range, ":enemy_village_no", villages_begin, villages_end), #tr47
			...
			(lt, ":real_dist", 80),
                    ...
                    (try_begin),
                      (eq, ":sod_campaign_posture", sod_campaign_posture_raiding),
                      (try_begin),
                        (ge, ":sod_marshal_planning", 60),
                        (store_mul, ":value_bonus", reg0, ":sod_marshal_planning"),
                        (val_div, ":value_bonus", 8),
                        (val_add, ":score", ":value_bonus"),
                        (try_begin),
                          (gt, ":real_dist", 45),
                          (store_mul, ":deep_raid_penalty", ":real_dist", ":sod_marshal_planning"),
                          (val_div, ":deep_raid_penalty", 5),
                          (val_sub, ":score", ":deep_raid_penalty"),
                        (try_end),
                      (else_try),
                        (lt, ":sod_marshal_planning", 35),
                        (gt, ":real_dist", 45),
                        (store_mul, ":deep_raid_bonus", ":real_dist", ":sod_marshal_aggression"),
                        (val_div, ":deep_raid_bonus", 5),
                        (val_add, ":score", ":deep_raid_bonus"),
                      (try_end),
                    (try_end),
					(else_try),
...
########## FINAL CHANCES ADJUSTMENTS	
	...
	(try_begin), #tr90
     ...
     (call_script, "script_sod_faction_apply_posture_to_lord_chances", ":troop_no", ":chance_move_to_home_center", ":target_move_to_home_center", ":chance_raid_around_center", ":chance_besiege_enemy_center", ":target_besiege_enemy_center", ":chance_patrol_around_center", ":target_raid_around_center"),
     (assign, ":chance_move_to_home_center", reg0),
     (assign, ":chance_raid_around_center", reg1),
     (assign, ":chance_besiege_enemy_center", reg2),
     (assign, ":chance_patrol_around_center", reg3),
     (assign, reg9, reg4),
     ...
     (try_begin),
       (ge, reg9, 55),
       (gt, ":target_move_to_home_center", 0),
       (val_mul, ":chance_move_to_home_center", 3),
       (val_div, ":chance_raid_around_center", 2),
       (val_div, ":chance_besiege_enemy_center", 2),
     (else_try),
       (is_between, ":sod_campaign_health", 1, 35),
       (gt, ":target_move_to_home_center", 0),
       (val_mul, ":chance_move_to_home_center", 3),
       (val_div, ":chance_raid_around_center", 2),
       (val_div, ":chance_besiege_enemy_center", 4),
       (val_div, ":chance_patrol_around_center", 2),
     (else_try),
       (is_between, ":sod_campaign_health", 35, 55),
       (gt, ":target_move_to_home_center", 0),
       (val_mul, ":chance_move_to_home_center", 3),
       (val_div, ":chance_move_to_home_center", 2),
       (val_mul, ":chance_raid_around_center", 4),
       (val_div, ":chance_raid_around_center", 5),
       (val_div, ":chance_besiege_enemy_center", 2),
     (else_try),
       (ge, ":sod_campaign_health", 75),
       (gt, ":chance_besiege_enemy_center", 1),
       (val_mul, ":chance_besiege_enemy_center", 5),
       (val_div, ":chance_besiege_enemy_center", 4),
       (val_mul, ":chance_patrol_around_center", 6),
       (val_div, ":chance_patrol_around_center", 5),
     (try_end),

     # Defensive patrols should stay in owned space. If an old AI object or
     # stay-here branch points at another realm's friendly/neutral center, drop
     # the patrol chance; hostile centers are handled by active war pressure.
     (try_begin),
       (gt, ":chance_patrol_around_center", 0),
       (try_begin),
         (neg|is_between, ":target_patrol_around_center", centers_begin, centers_end),
         ...
         (assign, ":target_patrol_around_center", -1),
       (else_try),
         (store_faction_of_party, ":patrol_target_faction", ":target_patrol_around_center"),
         (neq, ":patrol_target_faction", ":faction_no"),
         (call_script, "script_get_center_faction_relation_including_player", ":target_patrol_around_center", ":faction_no"),
         (ge, reg0, 0),
         (assign, ":chance_patrol_around_center", 0),
         (assign, ":target_patrol_around_center", -1),
       (try_end),
     (try_end),
	   ...
	   (assign, reg10, ":sod_campaign_health"),
	   (display_log_message, "@{s12} final chances total {reg3} home {reg4} other {reg5} besiege {reg6} patrol {reg7} recruit {reg8} pressure {reg9} faction_health {reg10}", debug_color),  
	   (try_end), #end tr92	
        ...
        (else_try),
          ...
          (lt, ":random_no", 0),
          (try_begin),
            (is_between, ":target_raid_around_center", centers_begin, centers_end),
            (store_current_day, ":cur_day"),
            (troop_set_slot, ":troop_no", slot_troop_sod_lord_last_profitable_raid_target, ":target_raid_around_center"),
            (troop_set_slot, ":troop_no", slot_troop_sod_lord_last_profitable_raid_day, ":cur_day"),
          (try_end),
