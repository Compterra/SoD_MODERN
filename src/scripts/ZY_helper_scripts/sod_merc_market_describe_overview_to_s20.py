...
("sod_merc_market_describe_overview_to_s20",
 [
   ...
   (str_store_string, s50, "@{s50}^^Castle mercenary guild halls:"),
   (try_for_range, ":kingdom_faction", kingdoms_begin, kingdoms_end),
     (neq, ":kingdom_faction", "fac_kingdom_6"),
     (faction_slot_eq, ":kingdom_faction", slot_faction_state, sfs_active),
     ...
     (try_begin),
       ...
       (str_store_string, s50, "@{s50}^{s51}: {reg51} hall(s), {reg52} local hires, pact-backed by {s52}."),
     (else_try),
       (str_store_string, s50, "@{s50}^{s51}: {reg51} hall(s), {reg52} local hires, independent vanilla offices."),
     (try_end),
   (try_end),
