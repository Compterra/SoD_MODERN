...
("sod_threat_board_describe_active_contract",
 [
  ...
  (call_script, "script_sod_threat_board_get_archetype", ":archetype"),
   ...
   (try_begin),
     (eq, ":ready", 1),
     (str_store_string, s1, "@Job Board Contract^^Sponsor: {s2}^{s8}^Target: {s3}^Tier {reg3}; ready to claim.^Reward: {reg2} denars, {reg6} XP, +{reg4} relation.^Note: {s4}"),
   (else_try),
     (str_store_string, s1, "@Job Board Contract^^Sponsor: {s2}^{s8}^Target: {s3}^Tier {reg3}; {reg1} days left.^Reward: {reg2} denars, {reg6} XP, +{reg4} relation.^Note: {s4}"),
   (try_end),
  (else_try),
